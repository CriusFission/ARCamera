...
import at.researchstudio.pca.mikhail.arcore.recorder.R;
...
public class ArpActivity extends AppCompatActivity implements GLSurfaceView.Renderer, VideoRecorder.VideoRecorderListener {
    ...
    public void onDrawFrame(GL10 gl) {
        ...
        try {
            ...
            // throttle the rendering to the camera frame rate.
      ...
      //      Log.d(TAG, "Available focal lengths: " + Arrays.toString(distortion));
            ...
            if (mRecorder!= null && mRecorder.isRecording() && posesFileCreated && camera.getTrackingState()==TrackingState.TRACKING) {
                ...
                //bufWriter.append("" + frameId  + " " + camera.getPose() + " " + getIntrinsicsAsString(camera.getTextureIntrinsics())
                );
                bufWriter.newLine();
                frameId++;
    ...
    public void clickToggleRecording(View view) {
        ...
        if (mRecorder == null) {
            ...
            try {
                ...
                mRecorder = new VideoRecorder(width,
                        height,
